...
post '/master/findings/new' do
    ...
    data.delete("msf_ref")
    puts data
    #@finding = CL_Library_finding.new(data)
    
    #@finding.save

    # find the id of the newly created finding so we can link mappings to it
    @newfinding = CL_Library_finding.first(:pTitle => data["title"], :order => [:pId.desc], :limit => 1)
...
get '/master/findings/:id/edit' do
    ...
    # Query for Finding
    @finding = CL_Library_finding.first(:pId => id)
	  @templates = Xslt.all()
...
post '/master/findings/:id/edit' do
    ...
    @finding = TemplatCL_Library_findingeFindings.first(:pId => id)
